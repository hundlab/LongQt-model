...
void Br04::updateIto_f() {
    ...
    iTo_f = ItofFactor*gkto_f*gate.a_to_f*gate.a_to_f*gate.a_to_f*gate.i_to_f*(vOld-EK);  //gKto_f_apex or septum???? TJH
...
void Br04::updateIto_s() {
    ...
    iTo_s = ItosFactor*gkto_s*gate.a_to_s*gate.i_to_s*(vOld-EK); //gKto_s_apex or septum??? TJH
};
...
void Br04::updateIk1() {
    ...
    iK1 = Ik1Factor*0.2938*(kO/(kO+0.210))*((vOld-EK)/(1+exp(0.0896*(vOld-EK))));
};
...
void Br04::updateIkur(){
    ...
    iKur = IkurFactor*gkur*gate.a_ur*gate.i_ur*(vOld-EK);  //apex vs. septum??? TJH
};
...
void Br04::updateIkss() {
    ...
    iKss = IkssFactor*gkss*gate.a_Kss*(vOld-EK);  //apex vs. septum??? TJH
};
...
void Br04::updateInak() {
    ...
    iNak = InakFactor*condfact*(inakmax*fNak/(1+pow((KmNai/naI),1.5)))*(kO/(kO+KmKo));
}
...
void Br04::updateInaca() {
    ...
    iNaca = InacaFactor*(kNaca/(KmNa*KmNa*KmNa+naO*naO*naO))*(1/(KmCa+caO))*(1/(1+(ksat*exp((Eta-1)*vOld*FDAY/(RGAS*TEMP)))))*((exp(Eta*vOld*FDAY/(RGAS*TEMP))*naI*naI*naI*caO)-(exp((Eta-1)*vOld*FDAY/(RGAS*TEMP))*naO*naO*naO*caI));
}
...
void Br04::updateIns() {
    ...
    iNsk = InskFactor*inskbar/(1+(kmns/caI)*(kmns/caI)*(kmns/caI));
    iNsna = InsnaFactor*insnabar/(1+(kmns/caI)*(kmns/caI)*(kmns/caI));
}
...
void Br04::updateCaFlux() {
    ...
    J_up = JupFactor*iupcondfact*(v3*caI*caI)/(Km_up*Km_up+caI*caI);
    ...
    J_trpn = JtrpnFactor*(kh_open*caI*1000*(htrpnbar-caTrpn_high)-kh_close*caTrpn_high+kl_open*caI*1000*(ltrpnbar-caTrpn_low)-kl_close*caTrpn_low);
};
...
void Br04::makemap()
{
    ...
    pars["ItofFactor"] = &ItofFactor;
    pars["ItosFactor"] = &ItosFactor;
    ...
    pars["InskFactor"] = &InskFactor;
    pars["InsnaFactor"] = &InsnaFactor;
    pars["JrelFactor"] = &JrelFactor;
    pars["JleakFactor"] = &JleakFactor;
    pars["JxferFactor"] = &JxferFactor;
    pars["JupFactor"] = &JupFactor;
    pars["JtrFactor"] = &JtrFactor;
    pars["JtrpnFactor"] = &JtrpnFactor;
}
