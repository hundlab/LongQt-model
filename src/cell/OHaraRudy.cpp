...
OHaraRudy *OHaraRudy::clone() { return new OHaraRudy(*this); }
// External stimulus.
// default stimval -80
...
void OHaraRudy::updateINa() {
  ...
  double tm = 1.0 / (6.765 * exp((vOld + 11.64) / 34.77) +
                     8.552 * exp(-(vOld + 77.42) / 5.955));
  m = mss - (mss - m) * exp(-dt / tm);
  double hss = 1.0 / (1 + exp((vOld + 82.90) / 6.086));
  double thf = 1.0 / (1.432e-5 * exp(-(vOld + 1.196) / 6.285) +
                      6.149 * exp((vOld + 0.5096) / 20.27));
  double ths = 1.0 / (0.009794 * exp(-(vOld + 17.95) / 28.05) +
                      0.3343 * exp((vOld + 5.730) / 56.66));
  double Ahf = 0.99;
  double Ahs = 1.0 - Ahf;
  ...
  double jss = hss;
  double tj = 2.038 + 1.0 / (0.02136 * exp(-(vOld + 100.6) / 8.281) +
                             0.3052 * exp((vOld + 0.9941) / 38.45));
...
void OHaraRudy::updateIto() {
  ...
  double ta =
      1.0515 / (1.0 / (1.2089 * (1.0 + exp(-(vOld - 18.4099) / 29.3814))) +
                3.5 / (1.0 + exp((vOld + 100.0) / 29.3814)));
  ...
  double tiF = 4.562 + 1 / (0.3933 * exp((-(vOld + 100.0)) / 100.0) +
                            0.08004 * exp((vOld + 50.0) / 16.59));
  double tiS = 23.62 + 1 / (0.001416 * exp((-(vOld + 96.52)) / 59.05) +
                            1.780e-8 * exp((vOld + 114.1) / 8.079));
  tiF *= delta_epi;
  tiS *= delta_epi;
  ...
  double dti_develop = 1.354 + 1.0e-4 / (exp((vOld - 167.4) / 15.89) +
                                         exp(-(vOld - 12.23) / 0.2154));
...
void OHaraRudy::updateICa() {
  ...
  double td =
      0.6 + 1.0 / (exp(-0.05 * (vOld + 6.0)) + exp(0.09 * (vOld + 14.0)));
  d = dss - (dss - d) * exp(-dt / td);
  double fss = 1.0 / (1.0 + exp((vOld + 19.58) / 3.696));
  double tff = 7.0 + 1.0 / (0.0045 * exp(-(vOld + 20.0) / 10.0) +
                            0.0045 * exp((vOld + 20.0) / 10.0));
  double tfs = 1000.0 + 1.0 / (0.000035 * exp(-(vOld + 5.0) / 4.0) +
                               0.000035 * exp((vOld + 5.0) / 6.0));
  double Aff = 0.6;
  double Afs = 1.0 - Aff;
  ...
  double fcass = fss;
  double tfcaf = 7.0 + 1.0 / (0.04 * exp(-(vOld - 4.0) / 7.0) +
                              0.04 * exp((vOld - 4.0) / 7.0));
  double tfcas =
      100.0 + 1.0 / (0.00012 * exp(-vOld / 3.0) + 0.00012 * exp(vOld / 7.0));
  ...
  ICaL =
      IcalFactor *
      ((1.0 - fICaLp) * PCa * PhiCaL * d * (f * (1.0 - nca) + jca * fca * nca) +
       fICaLp * PCap * PhiCaL * d * (fp * (1.0 - nca) + jca * fcap * nca));
  ICaNa = IcalFactor * ((1.0 - fICaLp) * PCaNa * PhiCaNa * d *
                            (f * (1.0 - nca) + jca * fca * nca) +
                        fICaLp * PCaNap * PhiCaNa * d *
                            (fp * (1.0 - nca) + jca * fcap * nca));
  ICaK = IcalFactor *
         ((1.0 - fICaLp) * PCaK * PhiCaK * d *
              (f * (1.0 - nca) + jca * fca * nca) +
...
void OHaraRudy::updateIkr() {
  ...
  double txrf = 12.98 + 1.0 / (0.3652 * exp((vOld - 31.66) / 3.869) +
                               4.123e-5 * exp((-(vOld - 47.78)) / 20.38));
  double txrs = 1.865 + 1.0 / (0.06629 * exp((vOld - 34.70) / 7.355) +
                               1.128e-5 * exp((-(vOld - 29.74)) / 25.94));
  ...
  double rkr = 1.0 / (1.0 + exp((vOld + 55.0) / 75.0)) * 1.0 /
               (1.0 + exp((vOld - 10.0) / 30.0));
  double GKr = 0.046;
  if (celltype == epi) {
    GKr *= 1.3;
  }
...
void OHaraRudy::updateIKs() {
  ...
  double txs1 = 817.3 + 1.0 / (2.326e-4 * exp((vOld + 48.28) / 17.80) +
                               0.001292 * exp((-(vOld + 210.0)) / 230.0));
  xs1 = xs1ss - (xs1ss - xs1) * exp(-dt / txs1);
  double xs2ss = xs1ss;
  double txs2 = 1.0 / (0.01 * exp((vOld - 50.0) / 20.0) +
                       0.0193 * exp((-(vOld + 66.54)) / 31.0));
...
void OHaraRudy::updateIK1() {
  double xk1ss =
      1.0 /
      (1.0 + exp(-(vOld + 2.5538 * kO + 144.59) / (1.5692 * kO + 3.8115)));
  double txk1 =
      122.2 / (exp((-(vOld + 127.2)) / 20.36) + exp((vOld + 236.8) / 69.33));
...
void OHaraRudy::updateINaK() {
  ...
  double Knao =
      Knao0 * exp(((1.0 - delta) * vOld * FDAY) / (3.0 * RGAS * TEMP));
  double Kki = 0.5;
  double Kko = 0.3582;
  double MgADP = 0.05;
  double MgATP = 9.8;
...
void OHaraRudy::updateCurr() {
  ...
  iTotold = iTot;
  iTot = INa + INaL + Ito + ICaL + ICaNa + ICaK + IKr + IKs + IK1 + INaCa +
         INaK + INab + IKb + IpCa + ICab;
}
