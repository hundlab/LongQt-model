...
void FR::Initialize() {
  ...
  /* Ion Concentrations */
  taudiff = 1000;  // Diffusion Constant for Ion Movement from Bulk Medium to
                   // Cleft Space
  ...
  // tauoff = 0.5;
  csqnth =
      8.75;  // Threshold for release of Ca from CSQN due to JSR overload (mM)
  gmaxrel =
      150;  // Max. rate constant of Ca release from JSR due to overload (ms^-1)
  ...
  katp =
      0.250;  // Half-maximal saturation point of ATP-sensitive K current (mM)

  /* Sodium-Calcium Exchanger V-S */

  c1 = .00025;   // Scaling factor for inaca (uA/uF)
  c2 = 0.0001;   // Half-saturation concentration of NaCa exhanger (mM)
  gammas = .15;  // Position of energy barrier controlling voltage dependance of
                 // inaca
...
FR* FR::clone() { return new FR(*this); }
...
void FR::comp_ina() {
  ...
  if (vOld < -40) {
    ...
    aj = (-127140 * exp(0.2444 * vOld) - 0.00003474 * exp(-0.04391 * vOld)) *
         ((vOld + 37.78) / (1 + exp(0.311 * (vOld + 79.23))));
...
void FR::comp_ical() {
  ...
  ibarca =
      pca * zca * zca * ((vOld * FDAY * FDAY) / (RGAS * TEMP)) *
      ((gacai * cai * exp((zca * vOld * FDAY) / (RGAS * TEMP)) - gacao * cao) /
       (exp((zca * vOld * FDAY) / (RGAS * TEMP)) - 1));
  ibarna =
      pna * zna * zna * ((vOld * FDAY * FDAY) / (RGAS * TEMP)) *
      ((ganai * nai * exp((zna * vOld * FDAY) / (RGAS * TEMP)) - ganao * nao) /
       (exp((zna * vOld * FDAY) / (RGAS * TEMP)) - 1));
  ibark = pk * zk * zk * ((vOld * FDAY * FDAY) / (RGAS * TEMP)) *
          ((gaki * ki * exp((zk * vOld * FDAY) / (RGAS * TEMP)) - gako * ko) /
           (exp((zk * vOld * FDAY) / (RGAS * TEMP)) - 1));
...
void FR::comp_ikr() {
  ...
  tauxr = 1 / (0.00138 * (vOld + 14.2) / (1 - exp(-0.123 * (vOld + 14.2))) +
               0.00061 * (vOld + 38.9) / (exp(0.145 * (vOld + 38.9)) - 1));
...
void FR::comp_iks() {
  ...
  xs2ss = xs1ss;
  tauxs1 = 1 / (0.0000719 * (vOld + 30) / (1 - exp(-0.148 * (vOld + 30))) +
                0.000131 * (vOld + 30) / (exp(0.0687 * (vOld + 30)) - 1));
  tauxs2 = 4 * tauxs1;
...
void FR::comp_iki() {
  ...
  bki = (0.49124 * exp(0.08032 * (vOld - eki + 5.476)) +
         exp(0.06175 * (vOld - eki - 594.31))) /
        (1 + exp(-0.5143 * (vOld - eki + 4.753)));
...
void FR::comp_ikatp() {
  /* Note: If you wish to use this current in your simulations, there are
   * additional       */
  /* changes which must be made to the code as detailed in Cardiovasc Res
   * 1997;35:256-272  */
...
void FR::comp_inaca() {
  inaca = inacaFactor * c1 * exp((gammas - 1) * vOld * FDAY / (RGAS * TEMP)) *
          ((exp(vOld * FDAY / (RGAS * TEMP)) * nai * nai * nai * cao -
            nao * nao * nao * cai) /
           (1 + c2 * exp((gammas - 1) * vOld * FDAY / (RGAS * TEMP)) *
                    (exp(vOld * FDAY / (RGAS * TEMP)) * nai * nai * nai * cao +
                     nao * nao * nao * cai)));
}
...
void FR::comp_inak() {
  ...
  fnak = 1 / (1 + 0.1245 * exp((-0.1 * vOld * FDAY) / (RGAS * TEMP)) +
              0.0365 * sigma * exp((-vOld * FDAY) / (RGAS * TEMP)));
  //
  inak = inakFactor * ibarnak * fnak * (1 / (1 + pow(kmnai / nai, 2))) *
         (ko / (ko + kmko));
}

void FR::comp_insca() {
  ibarnsna =
      pnsca * zna * zna * ((vOld * FDAY * FDAY) / (RGAS * TEMP)) *
      ((ganai * nai * exp((zna * vOld * FDAY) / (RGAS * TEMP)) - ganao * nao) /
       (exp((zna * vOld * FDAY) / (RGAS * TEMP)) - 1));
  ibarnsk = pnsca * zk * zk * ((vOld * FDAY * FDAY) / (RGAS * TEMP)) *
            ((gaki * ki * exp((zk * vOld * FDAY) / (RGAS * TEMP)) - gako * ko) /
             (exp((zk * vOld * FDAY) / (RGAS * TEMP)) - 1));
...
void FR::comp_ipca() { ipca = (ibarpca * cai) / (kmpca + cai); }
...
void FR::conc_nai() {
  ...
  // cell capacitance to get the correct units.  Since cell capacitance = 1
  // uF/cm^2, it doesn't explicitly appear in the equation below. This holds
  // true for the calculation of dki and dcai. */
...
void FR::conc_jsr() {
  ...
  greljsrol = irelFactor * grelbarjsrol * (1 - exp(-tjsrol / tauon)) *
              exp(-tjsrol / tauoff);
...
void FR::calc_itr() { itr = (nsr - jsr) / tautr; }

void FR::conc_cai() {
  dcai = -dt * (((iCat * ACap) / (Vmyo * zca * FDAY)) +
                ((iup - ileak) * Vnsr / Vmyo) - (irelcicr * Vjsr / Vmyo) -
                (ireljsrol * Vjsr / Vmyo));
  ...
  cai = (2 * gpig / 3) *
            cos(acos((9 * bmyo * cmyo - 2 * bmyo * bmyo * bmyo - 27 * dmyo) /
                     (2 * pow((bmyo * bmyo - 3 * cmyo), 1.5))) /
                3) -
        (bmyo / 3);
}
void FR::conc_cleft() {
...
void FR::updateCurr() {
  ...
  comp_ical();  // Calculates Currents through L-Type Ca Channel (output is
                // ilcatot)
  ...
  /*    std::cout << //inaca << "," << inak << "," << inab << "," << insna <<
     "," << insk << "," << ipca << "," << icab << std::endl;
                   //ikp << "," << ikna << "," << ikatp << "," << ito <<
     std::endl;
                   //ina << "," << ilcatot << "," << icat << "," << ikr << ","
     << iks << "," << iki << std::endl; cao<< "," << cai << "," << dcai <<
     std::endl;
                   //dt << "," << iup << "," << ileak << std::endl;
                   //iCat << "," << iKt << "," << iNat << "," << iTot << "," <<
     vOld << std::endl;
                   //insna << "," << insk << "," << ilca <<"," << ilcana << ","
     << ilcak << "," << cai << "," << iCat << std::endl;*/
}
...
void FR::makemap() {
  ...
  //   vars["kdkna"]=&kdkna;       // Dissociation constant for Na
  //   dependance(mM)
  ...
  //  (cm/s) vars["kmnsca"]=&kmnsca;      // Half-saturation concentration of
  //  NSCa channel (mM)
