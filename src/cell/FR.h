...
class FR : public Cell {
 ...
 public:
  ...
  double taudiff;  // Diffusion Constant for Ion Movement from Bulk Medium to
                   // Cleft Space
  ...
  double
      csqnth;  // Threshold for release of Ca from CSQN due to JSR overload (mM)
  double gmaxrel;  // Max. rate constant of Ca release from JSR due to overload
                   // (ms^-1)
  double grelbarjsrol;  // Rate constant of Ca release from JSR due to overload
                        // (ms^-1)
  ...
  double gpig;  // Tribute to all the guinea pigs killed for the advancement of
                // knowledge
  ...
  double gammas;  // Position of energy barrier controlling voltage dependance
                  // of inaca
