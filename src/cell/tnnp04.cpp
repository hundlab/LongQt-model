...
// Constructor
//######################################################
TNNP04Control::TNNP04Control() { this->Initialize(); }
...
void TNNP04Control::updateIcal() {
  ...
  tau_f = 1125 * exp(-1.0 * (vOld + 27) * (vOld + 27) / 300.0) +
          165.0 / (1.0 + exp((25.0 - vOld) / 10.0)) + 80.0;
  ...
  iCal = Icalfactor * gcal * condfact * Gate.d * Gate.f * Gate.fca * 4 *
         ...
         ((caI * exp(2 * vOld * FDAY / (RGAS * TEMP)) - 0.341 * caO) /
...
void TNNP04Control::updateIna() {
  ...
  m_inf =
      1 / (1 + exp((-56.86 - vOld) / 9.03) * (1 + exp((-56.86 - vOld) / 9.03)));
  alpha_m = 1 / (1 + exp((-60 - vOld) / 5));
  beta_m =
      (0.1 / (1 + exp((vOld + 35) / 5))) + (0.1 / (1 + exp((vOld - 50) / 200)));
  ...
  if (vOld >= -40) {
    ...
    beta_j =
        0.02424 * exp(-0.01052 * vOld) / (1 + exp(-0.1378 * (vOld + 40.14)));
  }
  ...
  iNa = Inafactor * gna * Gate.m * Gate.m * Gate.m * Gate.h * Gate.j *
        (vOld - ENa);
};
...
void TNNP04Control::updateIto() {
  ...
  tau_s = (85 * exp(-1 * (vOld + 45) * (vOld + 45) / 320)) +
          (5 / (1 + exp((vOld - 20) / 5))) + 3;
...
void TNNP04Control::updateIkr() {
  ...
  EK = RGAS * TEMP / FDAY * log(kO / kI);
  iKr =
      Ikrfactor * gkr * pow(kO / 5.4, 0.5) * Gate.xr1 * Gate.xr2 * (vOld - EK);
}
...
void TNNP04Control::updateIk1() {
  ...
  beta_K1 =
      (3 * exp(0.0002 * (vOld - EK + 100)) + exp(0.1 * (vOld - EK - 10))) /
      (1 + exp(-0.5 * (vOld - EK)));
...
void TNNP04Control::updateInaca() {
  ...
  iNaca = Inacafactor *
          ((kNaca * exp(gamma * vOld / RGAS / TEMP) * naI * naI * naI * caO) -
           (kNaca * exp((gamma - 1) * vOld * FDAY / RGAS / TEMP) * naO * naO *
            naO * caI * alpha)) /
          ((KmNai * KmNai * KmNai + naO * naO * naO) * (KmCa + caO) *
           (1 + (ksat * exp((gamma - 1) * vOld * FDAY / RGAS / TEMP))));
}
...
void TNNP04Control::updateInak() {
  ...
  iNak = Inakfactor * pnak * condfact * kO * naI /
         ((kO + KmK) * (naI + KmNa) *
          (1 + 0.1245 * exp(-0.1 * vOld * FDAY / RGAS / TEMP) +
           0.0353 * exp(-vOld * FDAY / RGAS / TEMP)));
}
...
void TNNP04Control::updateSRcurrents() {
  ...
  iRel = iRelfactor *
         (((a_rel * caSr * caSr) / (b_rel * b_rel + caSr * caSr)) + c_rel) *
         Gate.d * Gate.g;
  if (sponRelflag == 1)
    iRel = (((a_rel * caSr * caSr) / (b_rel * b_rel + caSr * caSr)) + c_rel) *
           Gate.g;
}
