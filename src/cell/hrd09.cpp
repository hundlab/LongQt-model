...
// ventricular model.
//######################################################
HRD09Control::HRD09Control() { this->Initialize(); };
...
void HRD09Control::updateIlca() {
  ...
  double kmca =
      0.002;  // Ca2+ half-saturation coeff. for Ca2+-dependent inactivation
  ...
  tauf = 22 +
         280 * exp(0.062 * (vOld + 28.3)) / (1 + exp(0.25 * (vOld + 28.3))) +
         deltat * camfact;
  ...
  maxIca =
      pca * 4 * (vOld) * (FDAY * FDAY / (RGAS * TEMP)) *
      (gamcai * caR * exp(2 * (vOld)*FDAY / (RGAS * TEMP)) - gamcao * caO) /
      (exp(2 * (vOld)*FDAY / (RGAS * TEMP)) - 1.0);
...
void HRD09Control::updateIcab() {
  ...
  maxicab =
      4 * (vOld) * (FDAY * FDAY / (RGAS * TEMP)) *
      (gamcai * caI * exp(2 * (vOld)*FDAY / (RGAS * TEMP)) - gamcao * caO) /
      (exp(2 * (vOld)*FDAY / (RGAS * TEMP)) - 1.0);
...
void HRD09Control::updateIks() {
  ...
  tauxs1 =
      1 / (5.0824E-5 * (vOld + 18.5172) / (1 - exp(-0.064 * (vOld + 18.5172))) +
           4.8824E-4 * (vOld + 18.5172) / (exp(0.064 * (vOld + 18.5172)) - 1));
  tauxs2 = 2 * tauxs1;
...
void HRD09Control::updateIkr() {
  ...
  tauxr =
      1.0 / (.6E-3 * (vOld - 1.7384) / (1 - exp(-.136 * (vOld - 1.7384))) +
             3.0E-4 * (vOld + 38.3608) / (exp(.1522 * (vOld + 38.3608)) - 1));
...
void HRD09Control::updateIk1() {
  ...
  Rate.bk1 = (.49124 * exp(.08032 * (vOld - Ek1 + 5.476)) +
              exp(.06175 * (vOld - Ek1 - 594.31))) /
             (1 + exp(-.5143 * (vOld - Ek1 + 4.753)));
...
void HRD09Control::updateIna() {
  ...
  if ((vOld - vshift) >= -40.0) {
    ...
    Rate.bh = 3.56 * exp(.079 * (vOld - vshift)) +
              (3.1E5) * exp(.35 * (vOld - vshift));
  }
  ...
  iNa = Inafactor * (MaxGNa * Gate.m * Gate.m * Gate.m * Gate.h * Gate.j) *
        (vOld - ENa);
};
...
void HRD09Control::updateInal() {
  ...
  iNal = Inalfactor * (0.0065 + camfact * deltag) * Gate.ml * Gate.ml *
         Gate.ml * Gate.hl * (vOld - ENa);
};
...
void HRD09Control::updateInaca() {
  ...
  iNacar =
      Inacafactorss * percr *
      (1 / (1 + (kmcaact / (caR)) * (kmcaact / (caR)))) *
      (vmax *
       (naI * naI * naI * caO * exp(eta * vOld * FDAY / (RGAS * TEMP)) -
        naO * naO * naO * caR * exp((eta - 1) * vOld * FDAY / (RGAS * TEMP))) /
       ((1 + ksat * exp((eta - 1) * vOld * FDAY / (RGAS * TEMP))) *
        (kmcao * naI * naI * naI + kmnao * kmnao * kmnao * caR +
         kmnai * kmnai * kmnai * caO * (1 + caR / kmcai) +
         kmcai * naO * naO * naO *
             (1 + naI * naI * naI / (kmnai * kmnai * kmnai)) +
         naI * naI * naI * caO + naO * naO * naO * caR)));
  // Na-Ca exchanger sensing bulk myocplasmic Ca
  iNaca =
      Inacafactorbulk * (1 - percr) *
      (1 / (1 + (kmcaact / (caI)) * (kmcaact / (caI)))) *
      (vmax *
       (naI * naI * naI * caO * exp(eta * vOld * FDAY / (RGAS * TEMP)) -
        naO * naO * naO * caI * exp((eta - 1) * vOld * FDAY / (RGAS * TEMP))) /
       ((1 + ksat * exp((eta - 1) * vOld * FDAY / (RGAS * TEMP))) *
        (kmcao * naI * naI * naI + kmnao * kmnao * kmnao * caI +
         kmnai * kmnai * kmnai * caO * (1 + caI / kmcai) +
         kmcai * naO * naO * naO *
             (1 + naI * naI * naI / (kmnai * kmnai * kmnai)) +
...
void HRD09Control::updateInacl() {
  ...
  ecl = -(RGAS * TEMP / (FDAY)) * log(clO / clI);
  iNacl = Inaclfactor * gnacl * (ENa - ecl) * (ENa - ecl) * (ENa - ecl) *
          (ENa - ecl) /
          ((ENa - ecl) * (ENa - ecl) * (ENa - ecl) * (ENa - ecl) +
           deltanct * deltanct * deltanct * deltanct);
};
...
void HRD09Control::updateInak() {
  ...
  fnak = 1.0 / (1.0 + .1245 * exp(-.1 * vOld * FDAY / (RGAS * TEMP)) +
                .0365 * sigma * exp(-vOld * FDAY / (RGAS * TEMP)));

  iNak = Inakfactor * inakbar * fnak * 1 / (1 + (kmnai / naI) * (kmnai / naI)) *
         kO / (kO + kmko);
};

// Transient outward K current
void HRD09Control::updateIto() {
  double tauzdv, zssdv, tauydv, tauy2dv, yssdv, yss2dv,
      rvdv;  // azdv,bzdv,aydv,bydv,ay2dv,by2dv,
  double Ek;
  ...
  iTo = Itofactor * MaxGto * Gate.a * Gate.a * Gate.a * Gate.i * Gate.i2 *
        rvdv * (vOld - Ek);
};
...
void HRD09Control::updateIto2() {
  ...
  ibar = pcl * (vOld)*FDAY * FDAY / (RGAS * TEMP) *
         (clI - clO * exp(-z * FDAY * (vOld) / (RGAS * TEMP))) /
         (1 - exp(-z * FDAY * (vOld) / (RGAS * TEMP)));
...
void HRD09Control::updateIrel() {
  ...
  irelinf = alpharel * iCa /
            (1 + (pow((kmrelinf - deltakmrelinf * camkfact) / caJsr, hrel)));
...
void HRD09Control::updateSrFlux() {
  ...
  iLeak = Ileakfactor * 0.75 * (iupbar / nsrbar) *
          (1 + deltaileak * camfactlk) * caNsr;
...
void HRD09Control::updateCai() {
  ...
  dcai = -dt * ((iCab + iPca - 2 * iNaca) * ACap / (Vmyo * 2.0 * FDAY) +
                (iUp - iLeak) * Vnsr / Vmyo - iDiff * Vss / Vmyo);
  ...
  c = (kmcmdn * kmtrpn) - (cait * (kmtrpn + kmcmdn)) + trpnbar * kmcmdn +
      cmdnbar * kmtrpn;
  d = -1 * kmtrpn * kmcmdn * cait;

  caI = (2.0 / 3.0) * sqrt(b * b - 3.0 * c) *
            cos(acos((9.0 * b * c - 2 * b * b * b - 27 * d) /
                     (2.0 * pow((b * b - 3.0 * c), 1.5))) /
                3.0) -
        b / 3.0;
...
void HRD09Control::updateCaSub() {
  ...
  dcar = dt * (-(iCa - 2 * iNacar) * ACap / (Vss * 2.0 * FDAY) +
               iRel * Vjsr / Vss - iDiff);
  ...
  c1 = (kmbsr * kmbsl) - (cart * (kmbsr + kmbsl)) + bsrbar * kmbsl +
       bslbar * kmbsr;
  d1 = -1 * kmbsr * kmbsl * cart;

  // Analytical solution for new value
  caR = (2.0 / 3.0) * sqrt(b1 * b1 - 3.0 * c1) *
            cos(acos((9.0 * b1 * c1 - 2 * b1 * b1 * b1 - 27 * d1) /
                     (2.0 * pow((b1 * b1 - 3.0 * c1), 1.5))) /
                3.0) -
        b1 / 3.0;
  caRt = caR + bsr + bsl;
};
...
void HRD09Control::updateCamk() {
  ...
  calmodulin =
      calmtotal * (caR * caR * caR * caR /
                   (caR * caR * caR * caR + 0.005 * 0.005 * 0.005 * 0.005));
