...
void Ksan::updateIst() {
  ...
  alphaqi =
      0.15 * 1.0 /
      (3100.0 * exp((vOld + 10.0) / 13.0) + 700.3 * exp((vOld + 10.0) / 70.0));
  betaqi = 0.15 * 1.0 /
               (95.7 * exp(-(vOld + 10.0) / 10.0) +
                50.0 * exp(-(vOld + 10.0) / 700.0)) +
           0.000229 / (1 + exp(-(vOld + 10.0) / 5.0));
...
void Ksan::updateICaT() {
  ...
  tau_dt = 1.0 / (1.068 * exp((vOld + 26.3) / 30.0) +
                  1.068 * exp(-(vOld + 26.3) / 30.0));
  dt_inf = 1.0 / (1.0 + exp(-(vOld + 26.0) / 6.0));
  Gate.dt = Gate.dt + dt * ((dt_inf - Gate.dt) / tau_dt);
  tau_ft = 1.0 / (0.0153 * exp(-(vOld + 61.7) / 83.3) +
                  0.015 * exp((vOld + 61.7) / 15.38));
...
void Ksan::updateIKr() {
  ...
  tau_ikr_act = 0.699821 / (0.003596 * exp((vOld) / 15.339290) +
                            0.000177 * exp(-(vOld) / 25.868423));
  Gate.ikr_act = Gate.ikr_act + dt * (ikr_act_inf - Gate.ikr_act) / tau_ikr_act;
  ikr_inact_inf = 1.0 / (1.0 + exp((vOld + 20.758474 - 4.0) / (19.0)));
  tau_ikr_inact =
      0.2 +
      0.9 * 1.0 / (0.1 * exp(vOld / 54.645) + 0.656 * exp(vOld / 106.157));
  Gate.ikr_inact =
      Gate.ikr_inact + dt * (ikr_inact_inf - Gate.ikr_inact) / tau_ikr_inact;
...
void Ksan::updateIKs() {
  ...
  double gks = 0.000299;
  double eks =
      ((RGAS * TEMP) / FDAY) * log((kO + 0.12 * naO) / (kI + 0.12 * naI));

  iks_act_inf = 1.0 / (1.0 + exp(-(vOld - 20.876040) / 11.852723));
  tau_iks_act =
      1000.0 / (13.097938 / (1.0 + exp(-(vOld - 48.910584) / 10.630272)) +
                exp(-(vOld) / 35.316539));
...
void Ksan::updateICaL() {
  ...
  if (fabs(vOld) <= 0.001) {
    alpha_dl =
        -28.39 * (vOld + 35.0) / (exp(-(vOld + 35.0) / 2.5) - 1.0) + 408.173;
  } else if (fabs(vOld + 35.0) <= 0.001) {
    alpha_dl = 70.975 - 84.9 * vOld / (exp(-0.208 * vOld) - 1.0);
  } else if (fabs(vOld) > 0.001 && fabs(vOld + 35.0) > 0.001) {
    alpha_dl = -28.39 * (vOld + 35.0) / (exp(-(vOld + 35.0) / 2.5) - 1.0) -
               84.9 * vOld / (exp(-0.208 * vOld) - 1.0);
  }

  if (fabs(vOld - 5.0) <= 0.001)
    beta_dl = 28.575;
  else if (fabs(vOld - 5.0) > 0.001)
  ...
  tau_fl =
      (7.4 + 45.77 * exp(-0.5 * (vOld + 28.1) * (vOld + 28.1) / (11 * 11)));
  ...
  ical12 =
      ICaL12factor * gcal12 * Gate.fl12 * Gate.dl12 * Gate.fca * (vOld - ecal);
  ical13 =
      ICaL13factor * gcal13 * Gate.fl13 * Gate.dl13 * Gate.fca * (vOld - ecal);
}
/**INa**********************************************************************/
void Ksan::updateINa() {
  double fna;
  ...
  fna = (9.52e-02 * exp(-6.3e-2 * (vOld + 34.4)) /
         (1 + 1.66 * exp(-0.225 * (vOld + 63.7)))) +
        8.69e-2;
  ...
  tau_m = 1000.0 * ((0.6247e-03 / (0.832 * exp(-0.335 * (vOld + 56.7)) +
                                   0.627 * exp(0.082 * (vOld + 65.01)))) +
                    0.0000492);
  tau_h = 1000.0 * (((3.717e-06 * exp(-0.2815 * (vOld + 17.11))) /
                     (1 + 0.003732 * exp(-0.3426 * (vOld + 37.76)))) +
                    0.0005977);
  tau_j = 1000.0 * (((0.00000003186 * exp(-0.6219 * (vOld + 18.8))) /
                     (1 + 0.00007189 * exp(-0.6683 * (vOld + 34.07)))) +
                    0.003556);
  ...
  tau_mr = 1000.0 * ((0.6247e-03 / (0.832 * exp(-0.335 * (vOld + 56.7)) +
                                    0.627 * exp(0.082 * (vOld + 65.01)))) +
                     0.0000492);
  tau_hr = 1000.0 * (((3.717e-06 * exp(-0.2815 * (vOld + 17.11))) /
                      (1 + 0.003732 * exp(-0.3426 * (vOld + 37.76)))) +
                     0.0005977);
  tau_jr = 1000.0 * (((0.00000003186 * exp(-0.6219 * (vOld + 18.8))) /
                      (1 + 0.00007189 * exp(-0.6683 * (vOld + 34.07)))) +
                     0.003556);
  ...
  if (fabs(vOld) > 0.005)
    ina_ttxs = INafactorxs * gna_ttxs * Gate.m_ttxs * Gate.m_ttxs *
               Gate.m_ttxs * hs * naO * (FDAY * FDAY / (RGAS * TEMP)) *
               ((exp((vOld - ena) * FDAY / (RGAS * TEMP)) - 1.0) /
                (exp(vOld * FDAY / (RGAS * TEMP)) - 1.0)) *
               vOld;
  else
    ina_ttxs = INafactorxs * gna_ttxs * Gate.m_ttxs * Gate.m_ttxs *
               Gate.m_ttxs * hs * naO * FDAY *
               ((exp((vOld - ena) * FDAY / (RGAS * TEMP)) - 1.0));
  if (fabs(vOld) > 0.005)
    ina_ttxr = INafactorxr * gna_ttxr * Gate.m_ttxr * Gate.m_ttxr *
               Gate.m_ttxr * hsr * naO * (FDAY * FDAY / (RGAS * TEMP)) *
               ((exp((vOld - enattxr) * FDAY / (RGAS * TEMP)) - 1.0) /
                (exp(vOld * FDAY / (RGAS * TEMP)) - 1.0)) *
               vOld;
  else
    ina_ttxr = INafactorxr * gna_ttxr * Gate.m_ttxr * Gate.m_ttxr *
               Gate.m_ttxr * hsr * naO * FDAY *
               ((exp((vOld - enattxr) * FDAY / (RGAS * TEMP)) - 1.0));
}
/**If**************************************************************************/
void Ksan::updateIf() {
  ...
  tau_y_1_2 =
      1.5049 / (exp(-(vOld + 590.3) * 0.01094) + exp((vOld - 85.1) / 17.2));
...
void Ksan::updateConc() {
  ...
  Gate.resting =
      Gate.resting +
      dt * (kim * Gate.resting_inactivated - kisrca * caSub * Gate.resting -
            kosrca * caSub * caSub * Gate.resting + kom * Gate.open);

  Gate.open = Gate.open +
              dt * (kosrca * caSub * caSub * Gate.resting - kom * Gate.open -
                    kisrca * caSub * Gate.open + kim * Gate.inactivated);

  Gate.inactivated = Gate.inactivated +
                     dt * (kisrca * caSub * Gate.open - kim * Gate.inactivated -
                           kom * Gate.inactivated +
                           kosrca * caSub * caSub * Gate.resting_inactivated);

  Gate.resting_inactivated =
      Gate.resting_inactivated +
      dt * (kom * Gate.inactivated -
            kosrca * caSub * caSub * Gate.resting_inactivated -
            kim * Gate.resting_inactivated + kisrca * caSub * Gate.resting);

  Jrel = ks * Gate.open * (caJsr - caSub);
  Jup = Pup * (pow(caI / pumpkmf, pumphill) - pow(caNsr / pumpkmr, pumphill)) /
        (1.0 + pow(caI / pumpkmf, pumphill) + pow(caNsr / pumpkmr, pumphill));

  // Jup = Pup*(pow(caI/pumpkmf,pumphill) - pow(caNsr/pumpkmr,pumphill))/(1.0 +
  // pow(caI/pumpkmf,pumphill) + pow(caNsr/pumpkmr,pumphill));
  Jtr = (caNsr - caJsr) / Ttr;
  ...
  caSub =
      caSub + dt * ((-ca_flux + Jrel * Vjsr) / Vss - Jcadif - ConcCM * dFcms);
  caI = caI + dt * ((Jcadif * Vss - Jup * Vnsr) / Vmyo -
