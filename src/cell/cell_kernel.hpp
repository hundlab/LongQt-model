...
namespace LongQt {
// Macro to create a flag enum with a map of its values used by subclasses to
// create Options enum
#define EXPAND(x) x
...
#define _MAKE_MAP1(N, ...) \
  std::map<std::string, int> optsMap = {EXPAND(MP_STR_##N(__VA_ARGS__))};
#define _MAKE_MAP(N, ...) _MAKE_MAP1(N, __VA_ARGS__)

#define _MAKE_ENUM1(N, ...)                         \
  enum Options { EXPAND(EN_STR_##N(__VA_ARGS__)) }; \
  CLASS_FUNCTIONS
#define _MAKE_ENUM(N, ...) _MAKE_ENUM1(N, __VA_ARGS__)

#define _MAKE_OPTIONS1(N, ...) \
  _MAKE_ENUM(N, __VA_ARGS__) _MAKE_MAP(N, __VA_ARGS__)
...
#define OPERATORS(CLASS_NAME)                                     \
  inline CLASS_NAME::Options operator~(CLASS_NAME::Options a) {   \
    return (CLASS_NAME::Options) ~(int)a;                         \
  }                                                               \
  inline CLASS_NAME::Options operator|(CLASS_NAME::Options a,     \
                                       CLASS_NAME::Options b) {   \
...
#define MAKE_OPTIONS_FUNCTIONS(CLASS_NAME) \
  OPERATORS(CLASS_NAME) OPTIONS_FUNCTIONS(CLASS_NAME)
